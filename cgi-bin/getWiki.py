...
class Wiki(object):
    ...
    def searchwiki(self, input, lang, site):
        response = ''
        term = input.encode('utf-8', 'ignore')
        self.lang = lang
        categories = []
        self.site = site
        ...
        try:
            ...
            for option in e.options:
                ...
                if count >= 6:
                    break  
                try:
                    x = wikipedia.page(option.encode('utf-8', 'ignore'))
                    ...
                    if site:
                        response = response + "<li><a href=\"" + x.url + "\">" + x.title + "</a>: " + wikipedia.summary(option.encode('utf-8', 'ignore'), sentences=1) + "</li>"
                    else:
                        response = response + "* [" + x.title + "](" + self.formaturl(x.url) + "): " + wikipedia.summary(option.encode('utf-8', 'ignore'), sentences=1) + "  \n"
    ...
    def subRecommender(self, response, categories):
        ...
        if len(subs) > 0:
            if self.site:
                ...
                for sub in subs:
                    response = response + '<li><a href="http://www.reddit.com/r/' + sub[0] + '">/r/' + sub[0] + "</a> based on similar categories like " + sub[1] + "</li>"
                response = response + "</ul>"
            else:
                response = response + "\n\nBased on what kinds of articles subreddits search for with WikiBot, here are related subreddits:  \n"
                for sub in subs:
                    response = response + "/r/" + sub[0] + "based on similar categories like " + sub[1] + "  \n"
    ...
    def wikifooter(self, input):
        if self.site:
            return input.encode('utf-8', 'ignore')
        else:
